...
use crate::{GameState, ascii::{AsciiSheet, spawn_ascii_sprite}, fadeout::{create_fadeout, FadeoutTimer}};
...
fn test_exit_combat(
    ...
    timer: ResMut<FadeoutTimer>
) {
    if keyboard.just_pressed(KeyCode::Space) {
        println!("Changing to overworld");
        create_fadeout(&mut commands, GameState::Overworld, &ascii, timer);
    }
}